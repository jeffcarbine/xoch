...
If a job is active, use the `xoch job current --json` result from the workflow boundary.
...
2. create a small patch note under `[xoch-root]/work/patches/` (resolve `[xoch-root]` with `xoch config root`)
