...
If the engineer names an existing job ID (a directory already under `[xoch-root]/work/jobs/[job-id]/`, resolve `[xoch-root]` with `~/.xoch/bin/xoch-actions.js config root`) that is not currently active, this is a reopen, not a fresh job.
...
3. If its `status` is `closed`, reactivate it:
   ...
   ```bash
   ~/.xoch/bin/xoch-actions.js state set --job "[job-id]" --field status --value active
   ```
...
4. Check `current_step` from the state just read:
   ...
   - anything else, or `null` -- the job was closed after implementation finished (or was fully reviewed). New work on an old job shell means new requirements, not a continuation of the old plan. Move it back to `spec`:
     ...
     ```bash
     ~/.xoch/bin/xoch-actions.js state set --job "[job-id]" --field current_step --value spec
     ```
...
3. Derive the job's id and title from the one identifier given:
   ...
   ```bash
   ~/.xoch/bin/generate-job-id.js --id "[identifier]"
...
4. Create the job with the deterministic helper -- it also seeds `current_step: title` and projects it into `current.json`:
   ...
   ```bash
   ~/.xoch/bin/xoch-actions.js job open --id "[job-id]" --title "[title]" --arc "[arc-id or standalone]" --doc-scope "[scope]" --doc-path "[path]"
   ```
...
5. For a multi-project job:
   ...
   - Run `xoch-actions.js job current --json` from every selected repository. Never displace unrelated active work.
   - Create the canonical scope in the primary job:

     ```bash
     ~/.xoch/bin/project-scope.js create --job "[job-id]" --primary "[name]=[absolute path]" --participant "[name]=[absolute path]"
     ~/.xoch/bin/context-sync.js sync --scope "[primary job]/projects.json" --dry-run
     ~/.xoch/bin/context-sync.js sync --scope "[primary job]/projects.json"
     ```

     Repeat `--participant` for additional repositories. Set a participant's local current pointer only when it has no conflicting active job and the engineer wants the job active there: run `xoch-actions.js job set-current --job "[job-id]"` from that participant root.
...
6. If the engineer wants Xoch work state kept local while docs remain shareable, preview and confirm the gitignore update in each participating repository:
   ...
   ```bash
   ~/.xoch/bin/gitignore-actions.js ensure --mode shared-docs --dry-run
   ~/.xoch/bin/gitignore-actions.js ensure --mode shared-docs
   ```
...
- Prefer `~/.xoch/bin/xoch-actions.js` for deterministic file/folder actions.
