...
Use the `xoch job current --json` result from the command wrapper. Run it now if the result is not already available; it returns target-model JSON state or legacy pointer metadata.
