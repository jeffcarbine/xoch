...
- `pass` - acceptance is covered, the full suite passes (or its failures are explicitly waived as unrelated to this job), {{xoch-config:coverage.strictness required="coverage is complete on every job-touched file (100%, or fully-recorded documented exceptions per `coverage-gate.md`)" recommended="coverage is complete on every job-touched file, or any reported gap was explicitly accepted by the engineer (100%, a fully-recorded documented exception, or an accepted gap per `coverage-gate.md`)" default="coverage is complete on every job-touched file (100%, or fully-recorded documented exceptions per `coverage-gate.md`)"}}, and no blocking risks remain
- `pass_with_waivers` - remaining gaps other than code coverage (which {{xoch-config:coverage.strictness required="cannot be waived here, only handled via a documented exception" recommended="is instead handled by the engineer's live accept/fix decision or a documented exception" default="cannot be waived here, only handled via a documented exception"}}) are explicitly waived by the engineer
...
- Coverage: `[file]` - {{xoch-config:coverage.strictness required="[percentage/status; must be 100% for every job-touched file with code, or a documented exception per `coverage-gate.md` with the file/branch, investigation findings, and source/test comment locations]" recommended="[percentage/status; 100%, a documented exception per `coverage-gate.md`, or a gap the engineer explicitly accepted -- record which]" default="[percentage/status; must be 100% for every job-touched file with code, or a documented exception per `coverage-gate.md` with the file/branch, investigation findings, and source/test comment locations]"}}
