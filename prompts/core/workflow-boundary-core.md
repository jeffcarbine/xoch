...
2. If the engineer explicitly answers the pending workflow and invokes another command in the same message, finish the recorded wrap-up first. Write required artifacts, synchronize multi-project context, and run `xoch workflow complete`; only then begin the newly invoked command.
...
4. Never treat a new command as implicit abandonment. Use `xoch workflow abandon --job "[job-id]" --name "[workflow]" --reason "[engineer-approved reason]" --next "[command]"` only after explicit engineer direction.
