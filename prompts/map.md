...
```bash
xoch workflow update --job "[job-id]" --name xoch-map --stage awaiting_confirmation --pending apply_workspace_map
```
...
```bash
xoch workspace add --name "[project]" --path "[absolute path]"
xoch workspace remove --name "[project]"
xoch workspace validate
xoch workspace list --json
```
...
```bash
xoch workflow complete --job "[job-id]" --name xoch-map --next "[recommended or explicitly invoked command]"
```
